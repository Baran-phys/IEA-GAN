...
def random_flip(x, p):
    ...
    flip_mask = (
        flip_mask.type(torch.bool)
        .view(n, 1, 1, 1)
        .repeat(1, c, h, w)
        .to(x.device)
    )
    x_out[flip_mask] = torch.flip(x[flip_mask].view(-1, c, h, w), [3]).view(
        -1
    )
    return x_out
...
def random_translation(x, ratio):
    ...
    t_x = torch.randint(
        -max_t_x, max_t_x + 1, size=[x.shape[0], 1, 1], device=x.device
    )
    t_y = torch.randint(
        -max_t_y, max_t_y + 1, size=[x.shape[0], 1, 1], device=x.device
    )
    ...
    x_pad = F.pad(
        input=x, pad=[max_t_y, max_t_y, max_t_x, max_t_x], mode="reflect"
    )
    x = (
