...
class InceptionV3pxd(nn.Module):
    ...
    x: Image with values in range [0,1]
    """
...
def build_resizer(mode):
    if mode == "clean":
        return make_resizer("PIL", False, "bilinear", (299, 299))
...
def make_resizer(library, quantize_after, filter, output_size):
    if library == "PIL" and quantize_after:
        ...
        def func(x):
            x = resize_single_channel(x[:, :, 0]).astype(
                np.float32
            )  # Have to use this for training
            # x = [resize_single_channel(x[:, :, idx]) for idx in range(3)] #Have to use this for make_custom_stats
...
class ResizeDataset(torch.utils.data.Dataset):
    ...
    fn_resize: function that takes an np_array as input [0,1]
    """
    ...
    def __getitem__(self, i):
        ...
        if img_resized.dtype == "uint8":
            ...
            img_t = self.transforms(img_resized) / 255
...
def get_reference_statistics(name, res, mode="clean", split="test", metric="FID"):
    ...
    """
    modpath = pathlib.Path(os.path.dirname(mycleanfid.__file__))
    if split == "custom":
        res = "na"
    if metric == "FID":
        rel_path = modpath.joinpath(f"{name}_{mode}_{split}_{res}.npz".lower())
        stats = np.load(rel_path)
        mu, sigma = stats["mu"], stats["sigma"]
        return mu, sigma
    elif metric == "KID":
        rel_path = modpath.joinpath(f"{name}_{mode}_{split}_{res}_kid.npz".lower())
        stats = np.load(rel_path)
        return stats["feats"]
...
def get_model_features(
    ...
    for idx in range(num_iters):
        with torch.no_grad():
            ...
            img_batch = (
                torch.pow(256, img_batch).add_(-1).div_(255).clamp_(0, 1)
            )
